...
export const signup = async (req, res, next) => {
    const { email, phoneNumber, password, telegramId } = req.body;
    ...
    if (!phoneNumber) {
        return res.status(422).json({ message: "Missing phone number." });
    }
    if (!password) {
    ...
    try {
        ...
        // Validate email
        const userWithSameEmail = await User.findOne({ email }).exec();
...
export const generateOTP = async (req, res, next) => {
    ...
    const verifyOtp = {
        ...
        <div style="font-family: Poppins, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9f9f9; padding: 20px; border: 1px solid #ccc; border-radius: 5px;">
            <h1 style="font-size: 22px; font-weight: 500; color: #854CE6; text-align: center; margin-bottom: 30px;">Verify Your Placements Notifer Account</h1>
            <div style="background-color: #FFF; border: 1px solid #e5e5e5; border-radius: 5px; box-shadow: 0px 3px 6px rgba(0,0,0,0.05);">
                ...
                <div style="padding: 30px;">
                    ...
                    <p style="font-size: 14px; color: #666; margin-bottom: 20px;">Thank you for creating a Placements Notifier account. To activate your account, please enter the following verification code:</p>
                    <p style="font-size: 20px; font-weight: 500; color: #666; text-align: center; margin-bottom: 30px; color: #854CE6;">${req.app.locals.OTP}</p>
                    <p style="font-size: 12px; color: #666; margin-bottom: 20px;">Please enter this code in the Placements Notifer app to activate your account.</p>
                    <p style="font-size: 12px; color: #666; margin-bottom: 20px;">If you did not create a Placements Notifer account, please disregard this email.</p>
                </div>
            </div>
            <br>
            <p style="font-size: 16px; color: #666; margin-bottom: 20px; text-align: center;">Best regards,<br>The Placements Notifer Team</p>
        </div>
        `
    };
    ...
    const resetPasswordOtp = {
        from: 'placements@shrishail.io',
        to: email,
        subject: 'Placements Notifer Reset Password Verification',
        html: `
        <div style="font-family: Poppins, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f9f9f9; padding: 20px; border: 1px solid #ccc; border-radius: 5px;">
            <h1 style="font-size: 22px; font-weight: 500; color: #854CE6; text-align: center; margin-bottom: 30px;">Reset Your Placements Notifer Account Password</h1>
            <div style="background-color: #FFF; border: 1px solid #e5e5e5; border-radius: 5px; box-shadow: 0px 3px 6px rgba(0,0,0,0.05);">
                <div style="background-color: #854CE6; border-top-left-radius: 5px; border-top-right-radius: 5px; padding: 20px 0;">
                    <h2 style="font-size: 28px; font-weight: 500; color: #FFF; text-align: center; margin-bottom: 10px;">Verification Code</h2>
                    <h1 style="font-size: 32px; font-weight: 500; color: #FFF; text-align: center; margin-bottom: 20px;">${req.app.locals.OTP}</h1>
                ...
                <div style="padding: 30px;">
                    <p style="font-size: 14px; color: #666; margin-bottom: 20px;">Dear ${name},</p>
                    <p style="font-size: 14px; color: #666; margin-bottom: 20px;">To reset your Placements Notifer account password, please enter the following verification code:</p>
                    <p style="font-size: 20px; font-weight: 500; color: #666; text-align: center; margin-bottom: 30px; color: #854CE6;">${req.app.locals.OTP}</p>
                    <p style="font-size: 12px; color: #666; margin-bottom: 20px;">Please enter this code in the Placements Notifer app to reset your password.</p>
...
export const verifyOTP = async (req, res, next) => {
    ...
    if (parseInt(code) === parseInt(req.app.locals.OTP)) {
        ...
        res.status(200).send({ succes: true, message: "OTP verified" });
    }
