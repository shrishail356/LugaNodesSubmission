...
import { fetchTransactions, addToFavorites,getFavoriteTransactions  } from '../api/index'; 
...
const LatestTransactionsPage = () => {
  ...
  const searchInput = useRef(null);
    const fetchAndSetTransactions = async () => {
        ...
        if (selectedContract) {
            try {
                const response = await fetchTransactions(selectedContract.address);
    ...
    const fetchAndSetFavorites = async () => {
      try {
          ...
          setFavorites(response); // Set favorites list
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleFavoriteToggle = async (transaction) => {
      const isFavorite = favorites.some(fav => fav.hash === transaction.hash);
      if (!isFavorite) {
          try {
              const response = await addToFavorites(transaction);
              if (response && response.message !== 'Transaction is already marked as a favorite.') {
                  setFavorites([...favorites, { hash: transaction.hash }]);
                  toast.success("Transaction Marked as favorite.");
              } else {
                  toast.error('Transaction is already marked as a favorite.');
              }
          } catch (error) {
...
const columns = [
    {
        ...
        width: 200, // Set width to enable scroll
    },
    {
        title: 'Age',
        ...
        width: 150, // Set width to enable scroll
    },
    {
        title: 'From Address',
              ...
              return (
                  <div>
                      <Button
                          ...
                          onClick={() => handleFavoriteToggle(transaction)}
                      />
                  </div>
              );
          },
    ...
    return (
      <DashboardMain>
          <div className="desktop-view">
            <Heading>Latest 1000 Transactions...</Heading>
